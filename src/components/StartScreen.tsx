  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-12">
      ...
      <p className="mt-3 text-gray-500">
        Type formulas in LaTeX. Equivalent notation is accepted automatically when the meaning matches.
      </p>
            ...
            >
              <div className="flex items-center justify-between">
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${style.badge}`}>
                  ×{info.multiplier.toFixed(1)} multiplier
                </span>
              </div>
      ...
      <div className="mt-10 text-sm text-gray-400 max-w-2xl text-center leading-relaxed">
        <p>
          10 questions per game. Time limits range from 20 to 60 seconds based on formula length. Press Enter to submit and Esc to pass.
        </p>
        <p className="mt-1 text-sky-500">
          Pass within the first 5 seconds to swap in another question without counting it.
        </p>
        <p className="mt-1">
          Supports physics (\dd, \grad, \rot...), bm (\bm), and braket (\bra, \ket, \braket) macros.
        </p>
      </div>
    </div>
  );
